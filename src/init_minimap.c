...
/*   Updated: 2023/02/16 15:17:18 by ewurstei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_minimap(t_vault *data)
{
	data->minimap->minimap = mlx_new_image(data->mlx,
			data->map->max_lenght * 65, data->map->lines * 65);
	draw_minimap(data);
	//contours MINIMAP
	full_line_minimap_hor(data, 0, 0xFF00FFFF);
	full_line_minimap_ver(data, 0, 0xFF00FFFF);
	full_line_minimap_ver(data, data->map->max_lenght * 65 - 1, 0xFF00FFFF);
	full_line_minimap_hor(data, data->map->lines * 65 - 1, 0xFF00FFFF);
	mlx_image_to_window(data->mlx,
		data->minimap->minimap, 0, data->height - data->map->lines * 65);
}
	...
	while (data->minimap->x < data->map->lines)
	{
		...
		while (data->minimap->y < data->map->max_lenght)
		{
			if (data->map->map[data->minimap->x][data->minimap->y] == '0')
				draw_tiles(data, data->minimap->x * 65, data->minimap->y * 65,
					0x6E99FFFF);
